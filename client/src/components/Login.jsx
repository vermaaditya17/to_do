...
export default function Login({ setAuth, toggleView }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post('https://to-do-dmhr.onrender.com/api/auth/login', { email, password });
  ...
  return (
    <div className="max-w-sm mx-auto mt-20 bg-white border border-zinc-200 p-8 rounded-2xl shadow-sm">
      <h2 className="text-xl font-black text-zinc-900 mb-6">Welcome Back</h2>
      ...
      <p className="text-xs text-zinc-400 mt-6 text-center">
        New here? <span onClick={toggleView} className="text-purple-600 font-semibold cursor-pointer hover:underline">Create an account</span>
      </p>
    </div>
  );
}